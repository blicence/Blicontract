...
import {ProxiesAddresses, PROXIES_ADDRESSES_FILENAME} from "../ProxiesAddresses";
 ...
 let  proxyAddresses: ProxiesAddresses = {
  ...
  function getProxyContractAddress(): string {
    ...
    return proxyAddresses.URI_GENERATOR_PROXY_ADDRESS;
  }
...
/*   await upgrades.deployImplementation(  customernft ,{
    kind: "uups",
  })  */
  const deploy =  await upgrades.upgradeProxy( PROXY_CONTRACT_ADDRESS, uriGenerator ,{
    kind: "uups",
  });
/*   .then(x=>{
    let xx= x.address;
    console.log("CustomerNftUpgradeable xxx ", xx);
   }); */   
 
   proxyAddresses.URI_GENERATOR_PROXY_ADDRESS = deploy.address;
   fs.writeFileSync(
